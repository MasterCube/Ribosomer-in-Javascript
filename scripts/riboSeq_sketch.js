...
var needsRedraw = true; // Initialize to true so first draw happens
var riboRRT_string;
var riboSP_string;
var riboFC_string = '';
...
//}
// Set needsRedraw = true when changes occur
// Existing mouse handlers - MODIFY like this:
function mouseMoved() {
  if (mouseX >= pxInf-12 && mouseX <= pxInf-12 + mRNAseq.length &&
      mouseY <= pyInfMap -40 && mouseY >= pySup) {
    needsRedraw = true;
  }
}
...
function setup(){
  ...
  //canvas_settings = createCanvas(1200, 800); // 1320, 750  1560,2050 (52850) for collagen
  ...
  for (i=0; i<iMax;i++){
    ...
    if (getItem("nonUniformmRNAabundanceStatus")=='true'){
      StoredTransCount[i] = int(getItem('readCopiesOf_'+str(i)));
    }
    else{
  ...
  //console.log('riboRRT_string', riboRRT_string);
  if(getItem('riboSeqFC') != null){
    ...
    //console.log('riboRRT_string length 1D', riboRRT_string.length);
  }
  ...
  if (typeof riboRRT_string == "string"){
    ...
    //console.log('riboRRT initializing check - BEEN HERE ? ?????????');
    ...
    for (tr=0; tr<riboRRT_string.length; tr++){
      //console.log('array1DofStrings[tr]', array1DofStrings[tr]);
      riboRRT[tr] = []; // second dimension of array initiated.
      //var arrayOfRRTonCodon = array1DofStrings[tr].split(/,/)
      //console.log('length in codons of this transcript read', riboRRT_string[tr].length);
      for (i_codon=0; i_codon<riboRRT_string[tr].length; i_codon++){
        //console.log('i_codon=', i_codon);
        //var rrt_float = float(arrayOfRRTonCodon[i_codon]);
        var rrt_float = float(riboRRT_string[tr][i_codon]);
        riboRRT[tr].push(rrt_float);
      }
    }
  }
  ...
  //console.log('true / false ?', typeof riboSP_string == "string");
  if (typeof riboSP_string == "string"){
    ...
    for (tr=0; tr<riboSP_string.length; tr++){
      //console.log('array1DofStrings[tr] SP', array1DofStrings[tr]);
      riboSP[tr] = []; // second dimension of array initiated.
      //var arrayOfSPonCodon = array1DofStrings[tr].split(/,/)
      for (i_codon=0; i_codon<riboSP_string[tr].length; i_codon++){
        var sp_float = float(riboSP_string[tr][i_codon]);
        riboSP[tr].push(sp_float);
      }
      //console.log('floated riboSP[tr]', tr, riboSP[tr]);
    }
  ...
  if (typeof riboFC_string == "string" && riboFC_string != ''){
    ...
    for (tr=0; tr < transcriptsFullNb; tr++){
      ...
      //console.log('riboFC_string', riboFC_string);
  ...
  // Existing button setup - MODIFY like this:
  buttonRRT.mousePressed(function() {
    buttonRRTOnOff(); // Keep your existing function
    needsRedraw = true; // Add this
  });

  buttonSP.mousePressed(function() {
    buttonSPOnOff(); // Keep your existing function
    needsRedraw = true; // Add this
  });

  buttonFC.mousePressed(function() {
    buttonFCOnOff(); // Keep your existing function
    needsRedraw = true; // Add this
  });
  ...
  // Add this RIGHT AFTER creating the slider:
  rangeSlider.changed(function() {
  needsRedraw = true;
  });
  ...
  // Initialize the default tagNameID
  getTagNameID();
  previousGeneID = '?';
  //console.log('initialized geneID', previousGeneID);

}// end setup

function draw(){// draw starts here
  // Only redraw when needed, not continuously
  ...
  //console.log('previousGeneID', previousGeneID);
  getTagNameID();
  let currentGeneID = tagInputName;
  //console.log('currentGeneID', currentGeneID);
  // Detect when gene actually changes
  if (tagInputName !== previousGeneID) {
    //console.log("Gene changed to:", tagInputName);
    // Add your additional change-handling code here
    getTagNameID();
    previousGeneID = tagInputName;
  }
  ...
  for (i_codon = 0; i_codon < mRNAseq.length/3; i_codon++){// number of codons in this transcript
    ...
    for (i = 0; i < readNumber; i++){
      //console.log('i=', i);
      //console.log('read_indices[i]=', read_indices[i]);
      //console.log('riboRRT', riboRRT);
