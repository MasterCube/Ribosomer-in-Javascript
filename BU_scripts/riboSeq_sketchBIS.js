...
function setup(){
  ...
  if (typeof riboRRT_string == "string"){
    ...
    //console.log('array1DofStrings.length', array1DofStrings.length);
    for (tr=0; tr<array1DofStrings.length; tr++){
      ...
      //console.log('floated riboRRT[tr]', tr, riboRRT[tr]);
    }
  }
...
function draw(){// draw starts here
  ...
  for (i_codon = 0; i_codon < mRNAseq.length/3; i_codon++){// number of codons in this transcript
    ...
    // do not divide by the number of reads because we will divide by the number of ribsomes!
    RRT.push(RRT_cumul);
    SP.push(SP_cumul);
